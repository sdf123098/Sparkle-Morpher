...
public class CustomRouletteEditorScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
        ...
        // 标题
        extractor.centeredText(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.title"), this.width / 2, 8, 0xFFFFFF);
        ...
        // 左面板标题
        extractor.text(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.available"), leftX, topY - 2, 0xFFAAAAAA);
        // 右面板标题
        extractor.text(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.custom_layout"), rightX, topY - 2, 0xFFAAAAAA);
    ...
    private void renderLeftPanel(GuiGraphicsExtractor g, int mouseX, int mouseY, int x, int y, int w, int h, int entryH) {
        ...
        for (int i = 0; i < availableAnimations.size(); i++) {
            ...
            g.text(this.font, label, x + 2, drawY + 1, color, false);
        }
    ...
    private void renderRightPanel(GuiGraphicsExtractor g, int mouseX, int mouseY, int x, int y, int w, int h, int entryH) {
        ...
        if (!pendingRootEntries.isEmpty()) {
            ...
            for (int i = 0; i < pendingRootEntries.size(); i++) {
                ...
                g.text(this.font, "  " + pendingRootEntries.get(i).displayLabel(), x + 2, drawY + 1, color, false);
            }
        }
        ...
        for (int gi = 0; gi < pendingGroups.size(); gi++) {
            ...
            if (headerY >= y && headerY < y + h) {
                ...
                g.text(this.font, Component.literal("[#" + group.name + "]"), x + 2, headerY + 1, headerColor, false);
            }
            logicalIdx++;
            ...
            for (int i = 0; i < group.animations.size(); i++) {
                ...
                int color = hovered ? 0xFFFF6666 : 0xFFCCCCCC;
                g.text(this.font, "  " + group.animations.get(i).displayLabel(), x + 2, drawY + 1, color, false);
            }
        }
