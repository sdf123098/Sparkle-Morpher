...
public final class ModelRepoClient {
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[SM][ResourceStation] Listing source finished sourceUrl={} entries={} elapsedMs={}",
                        sourceUrl, result.size(), elapsedMs(started));
            }
            return result;
        } catch (Exception e) {
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.warn("[SM][ResourceStation] Listing source failed sourceUrl={} elapsedMs={}", sourceUrl, elapsedMs(started), e);
            }
            throw e;
        }
    }
    ...
    public static byte[] download(ModelRepoEntry entry, ResourceStationConfig.State config) throws IOException {
        ...
        for (String url : candidates) {
            try {
                return read(url, config.timeoutMs(), config.maxDownloadBytes());
    ...
    public static byte[] download(ModelRepoEntry entry, ResourceStationConfig.State config, ProgressListener listener) throws IOException {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            try {
                return read(url, config.timeoutMs(), config.maxDownloadBytes(), listener, entry.size());
            } catch (IOException | RuntimeException e) {
                last = asIOException(e);
                YesSteveModel.LOGGER.warn("[SM] Resource download candidate failed: {}", url, e);
            }
    ...
    private static List<ModelRepoEntry> parseIndex(String indexUrl, String json) {
        ...
        for (JsonElement element : array) {
            ...
            if (url == null || url.isBlank()) {
                if (debugLogEnabled()) {
                    YesSteveModel.LOGGER.warn("[SM][ResourceStation] Skipping index entry without download URL indexUrl={} name={}", indexUrl, name);
                }
                continue;
            }
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[SM][ResourceStation] Parsed index indexUrl={} rawEntries={} importableEntries={}", indexUrl, array.size(), entries.size());
        }
        return entries;
    }
    ...
    private static byte[] read(String url, int timeoutMs, int maxBytes) throws IOException {
        return read(url, timeoutMs, maxBytes, null);
    }
    ...
    private static byte[] read(String url, int timeoutMs, int maxBytes, ProgressListener listener, long expectedBytes, boolean bypassCache) throws IOException {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[SM][ResourceStation] HTTP request started id={} url={} timeoutMs={} maxBytes={}",
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[SM][ResourceStation] HTTP response id={} code={} contentLength={} contentType={} elapsedMs={}",
                        requestId, code, length, contentType, elapsedMs(started));
            }
            ...
            try (InputStream in = connection.getInputStream(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                ...
                while ((read = in.read(buffer)) >= 0) {
                    ...
                    if (total > maxBytes) {
                        throw new IOException("File exceeds limit: downloaded=" + total + " maxBytes=" + maxBytes);
                    }
                    out.write(buffer, 0, read);
                    if (listener != null) {
                        listener.onProgress(total, progressTotal, bytesPerSecond(total, started, now));
                    }
                    if (progressTotal > 0 && total >= progressTotal) {
                        break;
                    }
                    long elapsedMs = elapsedMs(started, now);
                ...
                if (debugLogEnabled()) {
                    YesSteveModel.LOGGER.info("[SM][ResourceStation] HTTP request finished id={} bytes={} elapsedMs={}",
                            requestId, bytes.length, elapsedMs(started));
                }
                return bytes;
            }
        } catch (IOException | RuntimeException e) {
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.warn("[SM][ResourceStation] HTTP request failed id={} url={} elapsedMs={}",
                        requestId, url, elapsedMs(started), e);
            }
            throw e;
        } finally {
            if (connection != null) {
    ...
    private static List<String> downloadCandidates(ModelRepoEntry entry, ResourceStationConfig.State config) {
        ...
        if (entry.isGithubFile()) {
            String jsdelivr = "https://cdn.jsdelivr.net/gh/" + encPath(entry.githubOwner()) + "/" + encPath(entry.githubRepo()) + "@" + encPath(entry.githubBranch()) + "/" + encPath(entry.githubPath());
            if (config.mainlandChinaMode()) {
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[SM][ResourceStation] Probe candidate ok url={} bytes={} elapsedMs={} speed={}",
                        url, total, elapsed, formatSpeed(bytesPerSecond(total, started, System.nanoTime())));
            }
    ...
    private static List<String> indexCandidates(String url, ResourceStationConfig.State config) {
    ...
    private static void addGithubAcceleratorCandidates(Set<String> candidates, String url, ResourceStationConfig.State config) {
    ...
    private static void addGithubCdnCandidate(Set<String> candidates, String url) {
        URI uri;
        try {
            uri = URI.create(url);
        ...
        candidates.add("https://cdn.jsdelivr.net/gh/" + encPath(owner) + "/" + encPath(repo) + "@" + encPath(branch) + "/" + encPath(path));
    }
    ...
    private static long bytesPerSecond(int bytes, long startedNanos, long nowNanos) {
