...
public class CustomPlayerItemInHandLayer extends GeoLayerRenderer<CustomPlayerEntity> {
    ...
    private boolean applyItemBoneTransform(HumanoidArm humanoidArm, PoseStack poseStack, AnimatedGeoModel model, ItemStack itemStack, boolean importedPlayerModel) {
        ...
        List<? extends IBone> locatorHierarchy = humanoidArm == HumanoidArm.LEFT ? model.leftHandBones() : model.rightHandBones();
        return applyItemLocatorTransform(itemStack, poseStack, locatorHierarchy, importedPlayerModel);
    }

    private boolean applyItemLocatorTransform(ItemStack itemStack, PoseStack poseStack, List<? extends IBone> locatorHierarchy, boolean importedPlayerModel) {
        ...
        if (shouldIgnoreHiddenLastLocatorScale(itemStack)) {
    ...
    private boolean shouldIgnoreHiddenLastLocatorScale(ItemStack itemStack) {
        return "shield".equals(InnerClassify.getItemType(itemStack));
    }
