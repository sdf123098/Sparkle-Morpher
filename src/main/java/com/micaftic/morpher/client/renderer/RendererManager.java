...
public class RendererManager {
    private static CustomPlayerRenderer pr; private static ProjectileRenderer pjr; private static HandItemRenderer hr; private static VehicleRenderer vr; private static MaidEntityRenderer mr;
    ...
    @SubscribeEvent public static void onReload(AddReloadListenerEvent event) {
        event.addListener(new net.minecraft.server.packs.resources.ResourceManagerReloadListener() {
            ...
            @Override public void onResourceManagerReload(ResourceManager rm) { pr = null; pjr = null; hr = null; vr = null; mr = null; }
        });
    }
    private static void init(ResourceManager rm) {
        ...
        pr = new CustomPlayerRenderer(ctx); pjr = new ProjectileRenderer(ctx); hr = new HandItemRenderer(); vr = new VehicleRenderer(ctx); mr = new MaidEntityRenderer(ctx); SBackpackCompat.setupRenderLayers();
    }
    ...
    public static MaidEntityRenderer getMaidRenderer() { if (mr == null) init(Minecraft.getInstance().getResourceManager()); return mr; }
}
