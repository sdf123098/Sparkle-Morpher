...
public abstract class GeoReplacedEntityRenderer<TEntity extends LivingEntity, T extends LivingAnimatable<TEntity>> extends LivingEntityRenderer implements IGeoRenderer<T> {
    ...
    public boolean shouldShowName(TEntity entity) {
        ...
        return Minecraft.getInstance().getEntityRenderDispatcher().distanceToSqr(entity) < d * d && entity == Minecraft.getInstance().getEntityRenderDispatcher().crosshairPickEntity && entity.hasCustomName() && com.micaftic.morpher.client.renderer.RenderNameUtil.shouldRenderNames();
    }
