        ...
        ModelInfoCapability.get(sender).ifPresent(cap -> {
            AuthModelsCapability.get(sender).ifPresent(cap2 -> {
                ...
                if (!ServerModelManager.getServerModelInfo().containsKey(modelId)) {
                    YesSteveModel.LOGGER.warn("[SM] Reject model switch for '{}': unknown model '{}'", sender.getScoreboardName(), modelId);
                    cap.resetToDefault();
                    ...
                    YesSteveModel.LOGGER.warn("[SM] Reject model switch for '{}': model '{}' requires auth", sender.getScoreboardName(), modelId);
                    cap.resetToDefault();
                    PlayerModelSelectionStore.saveCurrentSelection(sender, cap);
                } else {
                    String textureId = ServerModelManager.resolveTextureOrDefault(modelId, message.textureId);
                    if (textureId == null) {
                        YesSteveModel.LOGGER.warn("[SM] Reject model switch for '{}': model '{}' has no valid texture", sender.getScoreboardName(), modelId);
                        cap.resetToDefault();
                        PlayerModelSelectionStore.saveCurrentSelection(sender, cap);
                    } else {
                        if (!textureId.equals(message.textureId)) {
                            YesSteveModel.LOGGER.warn("[SM] Replaced invalid texture '{}' for model '{}' on player '{}' with '{}'", message.textureId, modelId, sender.getScoreboardName(), textureId);
                        }
