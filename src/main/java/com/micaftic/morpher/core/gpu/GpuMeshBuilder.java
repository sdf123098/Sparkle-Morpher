...
public final class GpuMeshBuilder {
    ...
    private static MeshData buildJavaMesh(GeoModel model) {
        ...
        MeshData data = new MeshData(vertices, indices, vertexCount, indexCount, model.bakedBones.size());
    ...
    private static final class MeshData {
        ...
        MeshData(ByteBuffer vertices, ByteBuffer indices, int vertexCount, int indexCount, int boneCount) {
        ...
        void release() {
            if (vertices != null) {
                ResourceLifecycleStats.onDirectBufferFreed(null, (long) vertexCount * VERTEX_STRIDE);
