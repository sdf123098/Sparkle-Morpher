 ...
 *
 * <p>客户端：fabric 分支直接取 {@code Minecraft.getInstance().getConnection()}（1.21.1 无
 * MinecraftAccessor mixin）；SPM channel 协商校验由 {@code NetworkHandler.isConnectionValid}
 * 负责（{@code NetworkHandler.isClientConnected} 组合两者）。
