...
public class QueryBinding extends ContextBinding {
    ...
    private QueryBinding() {
        ...
        entityVar("eye_target_x_rotation", ctx -> com.micaftic.morpher.client.render.RenderContext.isOldHud()
                ? 0.0f : ctx.entity().getViewXRot(ctx.animationEvent().getPartialTick()));
        entityVar("eye_target_y_rotation", ctx -> com.micaftic.morpher.client.render.RenderContext.isOldHud()
                ? 0.0f : ctx.entity().getViewYRot(ctx.animationEvent().getPartialTick()));
        ...
        livingEntityVar("body_x_rotation", ctx -> com.micaftic.morpher.client.render.RenderContext.isOldHud()
                ? 0.0f : Mth.lerp(ctx.animationEvent().getFrameTime(), ctx.entity().xRotO, ctx.entity().getXRot()));
        livingEntityVar("body_y_rotation", ctx -> com.micaftic.morpher.client.render.RenderContext.isOldHud()
                ? 0.0f : Mth.wrapDegrees(Mth.lerp(ctx.animationEvent().getPartialTick(), ctx.entity().yBodyRotO, ctx.entity().yBodyRot)));
