    ...
    public AnimationEvent<?> processAnimationImpl(float partialTick, int capturedTickCount, boolean z) {
        ...
        if (!shouldSit && entity.isAlive() && livingEntity != null) {
            ...
            if (playerCapability != null && !playerCapability.isLocalPlayerModel()) {
                // Remote players: drive the limb cycle from the per-frame captured walkAnimation
                // state (the same source as the local player, partialTick-interpolated on the
                // render thread) so the walk cadence matches the actual movement. Fall back to the
                // position-tracker based physical speed only when the captured state is degenerate
                // while the entity is clearly moving (e.g. the entity was culled and walkAnimation
                // went stale).
