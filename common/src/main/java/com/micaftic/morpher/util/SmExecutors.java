...
public final class SmExecutors {
    ...
    public enum Pool {
        MODEL_PARSE, MODEL_IO, NETWORK_IO, DOWNLOAD_IO, SYNC_NETWORK, BACKGROUND
    }
    ...
    private static ThreadPoolExecutor create(Pool pool, int core, int max, int queueCapacity) {
        ...
        executor.setRejectedExecutionHandler(pool == Pool.DOWNLOAD_IO
    ...
    public static void shutdown() {
        for (ExecutorService executor : new ExecutorService[]{MODEL_PARSE, MODEL_IO, NETWORK_IO, DOWNLOAD_IO, SYNC_NETWORK, BACKGROUND}) {
            executor.shutdown();
        }
    }
