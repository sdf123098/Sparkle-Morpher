...
public class CustomRouletteEditorScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
        ...
        // 标题
        g.centeredText(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.title"), this.width / 2, 8, 0xFFFFFF);
        ...
        g.centeredText(this.font, Component.literal("[ " + targetLabel + " ]"), this.width / 2, 22, 0xFFFFCC00);
        ...
        // 左面板标题
        g.text(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.available"), leftX, topY - 2, 0xFFAAAAAA, true);
        // 右面板标题
        g.text(this.font, Component.translatable("gui.sparkle_morpher.roulette.editor.custom_layout"), rightX, topY - 2, 0xFFAAAAAA, true);

        renderLeftPanel(g, mouseX, mouseY, leftX, topY + 10, panelWidth, listHeight - 10, entryHeight);
        renderRightPanel(g, mouseX, mouseY, rightX, topY + 10, panelWidth, listHeight - 10, entryHeight);

        super.extractRenderState(extractor, mouseX, mouseY, partialTick);
    }

    private void renderLeftPanel(GuiGraphicsExtractor g, int mouseX, int mouseY, int x, int y, int w, int h, int entryH) {
        hoveredLeftIndex = -1;
        ...
        for (int i = 0; i < availableAnimations.size(); i++) {
            ...
            g.text(this.font, label, x + 2, drawY + 1, color, true);
        }

        g.disableScissor();
    }

    private void renderRightPanel(GuiGraphicsExtractor g, int mouseX, int mouseY, int x, int y, int w, int h, int entryH) {
        hoveredRightIndex = -1;
        hoveredRightKind = 0;
        g.enableScissor(x, y, x + w, y + h);
        int logicalIdx = 0;
        ...
        if (!pendingRootEntries.isEmpty()) {
            ...
            if (headerY >= y && headerY < y + h) {
                ...
                g.text(this.font, Component.literal("[Root]").withStyle(net.minecraft.ChatFormatting.GREEN), x + 2, headerY + 1,
                        selectedGroupIndex < 0 ? 0xFFFFCC00 : 0xFF55FF55, true);
            }
            logicalIdx++;
            for (int i = 0; i < pendingRootEntries.size(); i++) {
                ...
                g.text(this.font, "  " + pendingRootEntries.get(i).displayLabel(), x + 2, drawY + 1, color, true);
            }
        }
        ...
        for (int gi = 0; gi < pendingGroups.size(); gi++) {
            ...
            if (headerY >= y && headerY < y + h) {
                ...
                g.text(this.font, Component.literal("[#" + group.name + "]"), x + 2, headerY + 1, headerColor, true);
            }
            logicalIdx++;
            ...
            for (int i = 0; i < group.animations.size(); i++) {
                ...
                int color = hovered ? 0xFFFF6666 : 0xFFCCCCCC;
                g.text(this.font, "  " + group.animations.get(i).displayLabel(), x + 2, drawY + 1, color, true);
            }
        }
