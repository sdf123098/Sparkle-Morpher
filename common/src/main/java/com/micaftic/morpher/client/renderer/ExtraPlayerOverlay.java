...
public class ExtraPlayerOverlay implements HudOverlay {
    ...
    public void render(GuiGraphicsExtractor guiGraphics, Font font, float partialTick, int screenWidth, int screenHeight) {
        ...
        if ((localPlayer = (minecraft = Minecraft.getInstance()).player) == null
                || InputUtil.getCurrentScreen() instanceof HudLayoutScreen) {
            return;
        }
        ...
        ModelPreviewRenderer.renderPlayerOverlay(guiGraphics, localPlayer, position.x(), position.y(), scale, layout.getYaw(), -500, partialTick, false);
    }

    /**
     * 经典 HUD 小人可见性（§22.2 auto-hide / F5 hide / 动作可见性）。
     * 默认配置（全部关闭 / 0）恒返回 true，等价历史行为。
     */
