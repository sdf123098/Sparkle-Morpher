...
public final class MaidItemInHandLayer extends GeoLayerRenderer<MaidCapability> implements HeldItemLayer {
    ...
    private void renderHand(LivingEntity entity, AnimatedGeoModel model, HandLocatorProfile profile,
        ...
        if (hasDirectHandAnchor(model, arm)) {
            ...
            if (!transformed || profile.usesVanillaUseOrientation()) applyFallbackHandTransform(poseStack);
    ...
    private boolean applyItemBoneTransform(HumanoidArm arm, PoseStack poseStack, AnimatedGeoModel model, ItemStack item, HandLocatorProfile profile) {
        List<? extends com.micaftic.morpher.geckolib3.core.processor.IBone> locator = arm == HumanoidArm.LEFT ? model.leftHandBones() : model.rightHandBones();
        if (profile.usesSpecialHandLocatorSwordAnchor() && !item.isEmpty() && item.is(ItemTags.SWORDS)) {
            List<? extends com.micaftic.morpher.geckolib3.core.processor.IBone> swordLocator = arm == HumanoidArm.LEFT ? model.leftSwordBones() : model.rightSwordBones();
            if (swordLocator != null && !swordLocator.isEmpty()) locator = swordLocator;
        }
        return applyItemBoneTransform(poseStack, locator, profile);
    }

    private boolean applyItemBoneTransform(PoseStack poseStack, List<? extends com.micaftic.morpher.geckolib3.core.processor.IBone> locator, HandLocatorProfile profile) {
        if (locator == null || locator.isEmpty()) return false;
        if (profile.usesEquipmentLocatorTransform()) RenderUtils.prepMatrixForEquipmentLocator(poseStack, locator); else RenderUtils.prepMatrixForLocator(poseStack, locator);
        return true;
    }

    private boolean hasHandAnchor(AnimatedGeoModel model, HumanoidArm arm) { return hasDirectHandAnchor(model, arm) || hasHandChainAnchor(model, arm); }
    private boolean hasDirectHandAnchor(AnimatedGeoModel model, HumanoidArm arm) { return arm == HumanoidArm.LEFT ? !model.leftHandBones().isEmpty() : !model.rightHandBones().isEmpty(); }
    ...
    public void renderGltfThirdPersonItem(LivingEntity livingEntity, ItemStack itemStack, HumanoidArm humanoidArm, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, float partialTick) {
        ...
        renderItem(livingEntity, itemStack, humanoidArm == HumanoidArm.LEFT ? ItemDisplayContext.THIRD_PERSON_LEFT_HAND : ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, poseStack, packedLight);
    }
}
