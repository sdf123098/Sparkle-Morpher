...
public final class MaidItemInHandLayer extends GeoLayerRenderer<MaidCapability> implements HeldItemLayer {
    ...
    private boolean applyItemBoneTransform(HumanoidArm arm, PoseStack poseStack,
        ...
        if (profile.usesSpecialHandLocatorSwordAnchor() && isSwordItem(item)) {
    ...
    public void renderGltfThirdPersonItem(LivingEntity livingEntity, ItemStack itemStack,
        ...
        itemRenderer.renderItem(livingEntity, itemStack, getDisplayContext(humanoidArm),
                humanoidArm == HumanoidArm.LEFT, poseStack, bufferSource, packedLight);
    }

    private ItemDisplayContext getDisplayContext(HumanoidArm arm) {
        return arm == HumanoidArm.LEFT
                ? ItemDisplayContext.THIRD_PERSON_LEFT_HAND
                : ItemDisplayContext.THIRD_PERSON_RIGHT_HAND;
    }
