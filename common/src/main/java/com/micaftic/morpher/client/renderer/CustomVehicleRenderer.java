...
public class CustomVehicleRenderer {
    ...
    private static float getMinecartBodyRotation(AbstractMinecart minecart, float partialTick, float defaultYaw) {
        ...
        Vec3 interpPos = new Vec3(interpX, interpY, interpZ);

        float calculatedYaw = defaultYaw;

        if (interpPos != null) {
            Vec3 frontOffsetPos = new Vec3(interpX, interpY + 0.30000001192092896d, interpZ);
            Vec3 backOffsetPos = new Vec3(interpX, interpY - 0.30000001192092896d, interpZ);
