 ...
 *
 * <p>Everything that touches {@link Minecraft} (and therefore client-only
 * classes such as {@code LocalPlayer}) must stay in this class. The shared
 * compat classes are reachable from server-side mixins
 * ({@code MaidModelSync.handleInteraction}) and loading them on a dedicated
 * server must never resolve client classes — otherwise the server crashes
 * with {@code NoClassDefFoundError} during class verification.</p>
 */
public final class TouhouLittleMaidClientCompat {
    private TouhouLittleMaidClientCompat() {
    }

    public static void initOfficialCompat() {
        OfficialTouhouLittleMaidCompat.init(YesSteveModel.LOGGER);
    }
