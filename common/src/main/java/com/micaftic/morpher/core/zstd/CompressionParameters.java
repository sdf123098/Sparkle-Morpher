...
import static com.micaftic.morpher.core.zstd.Constants.MAX_WINDOW_LOG;
import static com.micaftic.morpher.core.zstd.Constants.MIN_WINDOW_LOG;
import static com.micaftic.morpher.core.zstd.Util.cycleLog;
...
class CompressionParameters
{
    ...
    private static CompressionParameters getDefaultParameters(int compressionLevel, long estimatedInputSize)
    {
        ...
        if (compressionLevel != 0) { // TODO: figure out better way to indicate default compression level
            row = Math.max(0, Math.min(compressionLevel, MAX_COMPRESSION_LEVEL));
        }
