...
public final class RoulettePanelStyle {
    ...
    public enum Glyph {
        MODEL(0, 0), SETTINGS(32, 0), ROULETTE(80, 0), APPLY(0, 16),
        RELOAD(48, 16), UP(64, 16), ROOT(80, 16), CLEAR(48, 32),
        CANCEL(64, 32), CLOSE(80, 32), SAVE(96, 32), DELETE(112, 32),
        CREATE(0, 48), MOVE(16, 48), PLUS(32, 48), MINUS(48, 48),
        INFO(96, 48), CHECK(112, 48);
    ...
    public static void glassPanel(GuiGraphics g, int x, int y, int w, int h) {
        blurGlass(g, x, y, w, h, 0x34F1FBFF, 8.0f);
        fill(g, x, y, w, h, GLASS);
        border(g, x, y, w, h, BORDER);
    }

    public static void secondaryGlassPanel(GuiGraphics g, int x, int y, int w, int h) {
        blurGlass(g, x, y, w, h, 0x58F7FBFF, 12.0f);
        fill(g, x, y, w, h, 0xD21F282E);
        border(g, x, y, w, h, 0xC8E4F5FF);
    }

    public static void blurGlass(GuiGraphics g, int x, int y, int w, int h, int tint, float radius) {
        if (w <= 0 || h <= 0) return;
        BlurStack.pushBlur(x, y, w, h, 0.0f, radius, tint);
        BlurStack.flush(g);
    }

    public static void iconButton(GuiGraphics g, int mouseX, int mouseY, int x, int y, Glyph glyph, boolean active) {
    ...
    public static void drawIcon(GuiGraphics g, Glyph glyph, int x, int y) {
        g.blit(MODEL_PANEL_ICONS, x, y, 16, 16, glyph.u, glyph.v, 16, 16, 128, 64);
    }
    ...
    public static String trim(Font font, String value, int maxWidth) {
        ...
        while (keep > 0 && font.width(text.substring(0, keep) + ellipsis) > maxWidth) keep--;
