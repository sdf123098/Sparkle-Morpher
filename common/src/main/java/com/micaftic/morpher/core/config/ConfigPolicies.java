...
/**
 * Stable policy boundary over the legacy Forge Config API values.
 * The legacy key names remain defined by the existing config classes.
 */
public final class ConfigPolicies {
    ...
    public static RenderPolicy render() {
        return new RenderPolicy(
                bool(GeneralConfig.DISABLE_SELF_MODEL, false),
                bool(GeneralConfig.DISABLE_OTHER_MODEL, false),
                bool(GeneralConfig.DISABLE_SELF_HANDS, false),
                bool(GeneralConfig.DISABLE_PROJECTILE_MODEL, false),
                bool(GeneralConfig.DISABLE_VEHICLE_MODEL, false),
                bool(GeneralConfig.DISABLE_EXTERNAL_FP_ANIM, false),
                bool(GeneralConfig.USE_COMPATIBILITY_RENDERER, false),
                bool(GeneralConfig.USE_GPU_RENDERER, true),
                bool(GeneralConfig.DISABLE_MODEL_GLOW_IN_SHADERPACK, true),
    ...
    public static MemoryPolicy memory() {
        return new MemoryPolicy(
                ...
                integer(GeneralConfig.MAX_CACHED_GPU_MODELS, 24),
                bool(GeneralConfig.LAZY_MODEL_LOADING, true),
                integer(GeneralConfig.MAX_RESIDENT_CPU_MODELS, 64),
                integer(GeneralConfig.UNUSED_MODEL_TTL_SECONDS, 300));
    }

    public static DiagnosticsPolicy diagnostics() {
        return new DiagnosticsPolicy(
                bool(GeneralConfig.ANIMATION_FRAME_PROFILER, false),
                bool(GeneralConfig.ANIMATION_DEBUG_LOG, false),
                bool(GeneralConfig.WARN_REPEATED_ANIMATION_EVALUATION, true),
                bool(GeneralConfig.RESOURCE_STATION_MONITOR_LOG, false),
                bool(GeneralConfig.NETWORK_ONLINE_DEBUG_LOG, false),
                bool(GeneralConfig.MODEL_MEMORY_PROFILER, false),
                bool(GeneralConfig.MODEL_IMPORT_PERFORMANCE_LOG, false),
                bool(GeneralConfig.INPUT_STATE_DEBUG_LOG, false));
    }

    public static PrivacyPolicy privacy() {
    ...
    public static AudioPolicy audio() {
        return new AudioPolicy(decimal(GeneralConfig.SOUND_VOLUME, 100.0));
    }
    ...
    private static double decimal(ForgeConfigSpec.DoubleValue value, double fallback) {
        try {
            return value == null ? fallback : value.get();
        } catch (Exception ignored) {
            return fallback;
    ...
    private static <T> T value(ForgeConfigSpec.ConfigValue<T> value, T fallback) {
        try {
            T result = value == null ? null : value.get();
    ...
    public record RenderPolicy(boolean disableSelfModel, boolean disableOtherModel, boolean disableSelfHands,
                               boolean disableProjectileModel, boolean disableVehicleModel,
                               boolean disableExternalFirstPersonAnimation, boolean useCompatibilityRenderer,
                               boolean useGpuRenderer, boolean disableModelGlowInShaderpack,
                               boolean animationDistanceLod) {
    }

    public record MemoryPolicy(int audioCacheMaxBytes, int maxCachedGpuModels, boolean lazyModelLoading,
                               int maxResidentCpuModels, int unusedModelTtlSeconds) {
    }

    public record DiagnosticsPolicy(boolean animationFrameProfiler, boolean animationDebugLog,
                                    boolean warnRepeatedAnimationEvaluation, boolean resourceStationMonitorLog,
                                    boolean networkOnlineDebugLog, boolean modelMemoryProfiler,
                                    boolean modelImportPerformanceLog, boolean inputStateDebugLog) {
    }
