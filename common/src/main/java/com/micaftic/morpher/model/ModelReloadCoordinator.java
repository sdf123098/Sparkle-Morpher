 ...
 *
 * <p>Only the newest request waiting behind an active reload is retained. A
 * completed result is published only when it still belongs to the newest
 * generation, so an older scan cannot overwrite a newer catalog.</p>
 */
public final class ModelReloadCoordinator<T> {
    ...
    /**
     * Queues a reload. If another reload is active, this replaces the pending
