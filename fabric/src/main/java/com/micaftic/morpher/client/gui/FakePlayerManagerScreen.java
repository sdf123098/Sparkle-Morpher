...
public final class FakePlayerManagerScreen extends Screen {
    ...
    private void draw(GuiGraphicsExtractor g, String value, int x, int y, int color) {
        g.text(font, Component.literal(value), x, y, color, false);
    }

    @Override
    public boolean mouseClicked(MouseButtonEvent event, boolean flag) {
        if (event.button() != 0) return super.mouseClicked(event, flag);
        int pw = Math.min(720, width - 28), ph = Math.min(430, height - 20);
        int x = (width - pw) / 2, y = (height - ph) / 2, sy = y + 48, fy = sy + 28;
        if (event.x() >= x + 18 && event.x() < x + pw - 18 && event.y() >= sy && event.y() < sy + 22) {
            searchFocused = true; return true;
        }
        for (int i = 0; i < 4; i++) {
            int bx = x + 18 + i * 72;
            if (event.x() >= bx && event.x() < bx + 66 && event.y() >= fy && event.y() < fy + 18) {
                filter = i; offset = 0; return true;
            }
        }
        int sx = x + pw - 150;
        if (event.x() >= sx && event.x() < x + pw - 18 && event.y() >= fy && event.y() < fy + 18) {
        ...
        for (int i = 0; i < count && offset + i < rows.size(); i++) {
            int ry = ly + i * ROW;
            if (event.x() >= x + 18 && event.x() < x + pw - 18
                    && event.y() >= ry && event.y() < ry + ROW - 2) {
                FakePlayerListEntry e = rows.get(offset + i);
                selectedUuid = e.uuid();
        ...
        if (rows.stream().anyMatch(e -> e.uuid().equals(selectedUuid))
                && event.x() >= x + 18 && event.x() < x + pw - 18
                && event.y() >= by && event.y() < by + 24) {
    ...
    public boolean charTyped(CharacterEvent event) {
        if (searchFocused && !Character.isISOControl(event.codepoint()) && query.length() < 64) {
            query += event.codepoint(); offset = 0; return true;
        }
        return super.charTyped(event);
    }
    @Override
    public boolean keyPressed(KeyEvent event) {
        if (searchFocused && event.key() == 259) {
            if (!query.isEmpty()) query = query.substring(0, query.length() - 1);
            offset = 0; return true;
        }
        if (event.key() == 257 || event.key() == 335) { searchFocused = false; return true; }
