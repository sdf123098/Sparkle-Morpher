...
public class ExtraPlayerRenderConfig {
    ...
    public static ModConfigSpec.EnumValue<HeadMode> CLASSIC_HUD_HEAD_MODE;

    /** 经典 HUD 小人锚点（默认 TOP_LEFT，等价历史「绝对像素位置」）。 */
    public static ModConfigSpec.EnumValue<PaperDollLayout.Anchor> CLASSIC_HUD_ANCHOR;

    /** 第三人称（F5）时隐藏经典 HUD 小人。 */
    public static ModConfigSpec.BooleanValue CLASSIC_HUD_HIDE_IN_THIRD_PERSON;

    /** 经典 HUD 小人空闲自动隐藏秒数（0 = 不自动隐藏）。 */
    public static ModConfigSpec.IntValue CLASSIC_HUD_AUTO_HIDE_IDLE_SECONDS;

    /** 经典 HUD 小人仅在动作中显示（动作可见性；优先于 auto-hide）。 */
    public static ModConfigSpec.BooleanValue CLASSIC_HUD_SHOW_ON_ACTION_ONLY;

    /** 经典 HUD 小人骑乘时随载具 yaw 对齐。 */
    public static ModConfigSpec.BooleanValue CLASSIC_HUD_ALIGN_WITH_VEHICLE;

    /**
     * 经典 HUD 小人头部朝向模式（路线图 §22.2 head mode）。
     *
     * <p>{@code STRAIGHT}：头部与身体同向（历史固定行为，默认）；
     * {@code FOLLOW}：头部跟随玩家真实视线（相对身体 yaw 限制 ±85°）。</p>
     */
